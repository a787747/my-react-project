...
| `/api/admin-users-data` | GET | none | `{ users: AdminUser[] }`; each user includes `join_date`, current-period fields and `period_scopes: [{period_id,period_name,period_type,period_status,start_date,end_date,scope_cutoff_date,has_period_row,is_in_scope,exclusion_reason,scope_override}]`; plus selector options | `useUsers`, `useScoreCoefficients`, `useScoreCalculation`, `useFinalScoresMatrix` |
| `/admin/save-user` | POST | existing row: **all** `{ id, full_name, email, job_title, role, work_category, department_id, grade_id, manager_id, join_date }`; missing field → 422 `INCOMPLETE_USER_ROW`. `join_date` is `YYYY-MM-DD` or empty. | `{ success: true, user, scope_results: [{period_id,period_name,outcome,…}], card_event_id, scope_events_written }`; date→out with evaluations → 409 `HIRE_DATE_SCOPE_HAS_EVALUATIONS`, whole card unchanged | `useUsers` (single + import loop) |
| `/api/admin/exclude-participant` | POST | `{ user_id, period_id, note? }` | 200 scope event; any evaluation/correction in period → hard 409 `HAS_EVALUATIONS` with received/self/given/correction counts (no confirmation override) | `UserModal` |
| `/api/admin/include-participant` | POST | `{ user_id, period_id, note? }` | 200; reverses admin/date exclusions, writes durable `included_by_admin`; termination and closed period refused | `UserModal` |
| `/api/admin/employee-events` | GET | required `?user_id=` | `{ success: true, events: [{source,event_type,user_id,period_id,actor_id,occurred_at,details}], total }`; union of card/scope/employment append-only logs | `UserModal` |
