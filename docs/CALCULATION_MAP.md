...
**`period_results` vs the `rating_*` columns (added 2026-08-21):** `final_rating` and `bonus_index` in `performance_db.period_results` are matrix quantities — mean over criterion final cells, and formula #3 over those same cells — while `rating_manager / rating_upward / rating_c_level_direct / rating_self` are archival per-source summaries of `evaluations.calculated_score`, a column the matrix never reads; **the two will not reconcile, by design**, since a per-source rating averages each evaluator's own mean over the criteria that evaluator graded, and they already diverge with a single evaluator plus one correction.
