...
| Backups | **Wrong database.** Daily cron 03:00 runs `/root/backups/epe/backup-performance-db.sh`, which dumps `-d postgres -n performance_db` — the **2025 archive**, not `epe_2026`. 10 dumps held, 14-day prune, log clean. **The live campaign database has no automated backup at all** (BUG-032). Off-host copy also still missing (BUG-014) |
...
5. **Off-host backup target** (Timeweb S3, write-only key) — outstanding since 13 Aug (BUG-014). Now paired with a sharper problem: **the daily on-host backup dumps the 2025 archive, not `epe_2026`** (BUG-032). Before H1 writes its first evaluation, the live database should be in the daily job.
...
| Item | Source | Row |
|---|---|---|
| Live campaign DB into the daily backup job; then the off-host copy | this hygiene pass | BUG-032, BUG-014 |
...
Operational: `LAUNCH_RUNBOOK_H1.md` (Alexander's one page), `INVITATION_WAVES.md` (now "single send"). Maps: `SERVER_MAP.md`, `FRONTEND_MAP.md`, `API_CONTRACT.md`, `CALCULATION_MAP.md`. Briefs: `docs/briefs/`. Decisions: `DECISIONS.md` (single register; `PROJECT_DECISIONS.md` is a pointer). Bugs: `bugs.md` (**20 open / 20 closed**). Progress: `PROGRESS.md`. Ports, names, and the throwaway-stand pattern: `PROJECT_RULES.md`. Migrations: `migrations/001…013`.
