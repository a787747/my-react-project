...
**As of:** 2026-08-26 (PRELAUNCH_BATCH_NIGHT) · **H1: active, not started**
...
| Frontend | This host. Current release **`20260825T194735Z`** (2026-08-25 19:47:35Z, PRELAUNCH_BATCH_NIGHT). **35** releases on disk; the rollback target is the previous one, `20260825T170810Z`. Deploy: `./scripts/deploy_epe_frontend.sh` — it now holds an exclusive lock locally and on the host and refuses a flip that started from a stale `current` (BUG-062), and its safety gates use `grep -r`, not `rg` (BUG-040). |
