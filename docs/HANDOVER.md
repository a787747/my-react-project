...
- **C-level reads the admin surfaces, HR reads the roster; neither writes (D-0826-6, corrected by D-0826-7, live 2026-08-26 13:47Z).** `api/admin-users-data` admits admin+hr+c_level (no salary column for anyone; `options.grades[].coefficient` stripped for hr); `GET api/score-coefficients` admits admin+c_level (the POST save stays admin); `manage-criteria` admits admin+c_level for `get` and refuses every non-admin `save`/`delete` 403 by role before the freeze check. Frontend: `/admin/final-scores`, `/admin/score-calculator`, `/admin` are `ReportingRoute` (HR → /hr/dashboard — BUG-013 closed); roster and criteria read-only below admin; refused/failed reads name their reason (BUG-042 closed). **Corrections are the ONE write route that admits c_level** — D-0820-7 stands (D-0826-7). Proven live: 151-cell role×route matrix PASS, browser walkthrough as real c_level and HR accounts, zero compensation keys in 23 non-admin payloads. Report: `docs/ROLE_ACCESS_HR_CLEVEL_2026-08-26.md` (§8 = the deploy).
...
- **Hire date and period scope are admin-editable in the employee card (D-0826-4/5).** `join_date` may be set to a date, emptied and restored. A change recomputes every non-closed participant row whose state is date-derived or has no reason; `terminated`, `excluded_by_admin` and the additive `scope_override` manual mark always win. Taking somebody out after any evaluation exists is a hard 409 with the received/self/given/correction counts — the old confirmation escape is gone; bringing somebody in is allowed. The automatic boundary is the final three calendar months: H1 ends 30 June, so 31 March is in and 1 April is out (`insufficient_tenure`). Period creation and card recompute use the same expression. The card names the outcome for every period and carries a per-period manual toggle; closed periods and annual containers are disabled on screen. Report: `docs/HIRE_DATE_AND_SCOPE_TOGGLE_2026-08-26.md`.
- **`join_date` is no longer a frozen employee column.** Its change through the admin card is an ordinary owner action and must not be reported as drift or an incident. The protected set for unrelated writes remains salary (`salary_current`, `salary_proposed`), credentials/session state (`password_hash`, `token_version`), policy flags (`can_evaluate`, `can_be_evaluated`), `employment_type` and `created_at`; a drift pass compares `join_date` too, but permits exactly the card edit it is proving.
- **Employee-change evidence is readable in one place, but is not one physical table.** The brief's premise was wrong: terminations already lived in `employment_events`, scope changes in `period_scope_events`. Migration 017 additively adds `employee_card_events` for old→new card fields and leaves both histories untouched; `GET /api/admin/employee-events?user_id=` unions all three with actor and time. No historical row was migrated.
- **`c_level_direct` is averaged across evaluators (D-0826-1, 2026-08-26).** When more than one
  ...
  criterion is refused since 2026-08-26 07:57Z with 422 `CRITERIA_NOT_APPLICABLE` (D-0826-3,
  closes BUG-073): corrections calibrate the manager channel, the C-level channel is calibrated by
  being averaged. Proven money-neutral by two closes of two copies of one dump (100/100 frozen
  rows identical) — `docs/PRELAUNCH_GATE_2026-08-26.md`.**
...
- **Periods on live — three rows:**
  ...
  | 2 | H1-2026 | `half_year` | `active` | true | **5** | 0 | 89 / **78** |
  ...
  **`2026-08-26 10:08:54.340312Z / 2`**; both annual rows are NULL.
  ...
  In scope re-measured **2026-08-26 13:37Z**: H1 **78 of 89**. Eleven are out — three terminated
  (39, 51, 66 — D-0825-7), two by hire date (31 Esenova, 35 Balova), four hand-marked under
  D-0825-11 (25 Asatryan, 64 Atayeva, 22 Chariyev, 63 Jumayeva), Jeren Atabayeva (49) excluded by
  Alexander at **10:11:52Z**, and **Aysoltan Kulmamedova (16) excluded by him at 12:31:49Z**, both
  `scope_override=excluded_by_admin`. The annual container still holds 86: an H1 exclusion does
  not remove a person from the year.
...
13. **Decided 2026-08-26 (D-0826-3) and shipped: the route refuses.** A `c_level` correction on a `c_level_only` criterion answers 422 `CRITERIA_NOT_APPLICABLE` — corrections calibrate the manager channel; the C-level channel is calibrated by being averaged (D-0826-1). BUG-073 closed; the two interpretive options (replace the mean / join the mean) remain available as a future decision, and nothing stored has to migrate because nothing can be stored. What remains open in the same territory: `score_corrections` has no evaluator in its unique key, so on the criteria where corrections DO count they are last-writer-wins across C-level people (recorded in BUG-073's row). Report: `docs/PRELAUNCH_GATE_2026-08-26.md` §5.
...
(2026-08-26 10:08:54.340312Z). The row reads «Идёт оценка»; H1 scope is 78 / 89 (13:37Z reading); employee tasks
are open.
...
· **`EMPLOYEE_SURFACES_POLISH_2026-08-26.md`** — the task icons link to their real pages,
