...
| `final_rating` | **per-person final = mean over criterion final cells**, each cell exactly as the matrix computes it: `mean(manager, mid?, c_level?)`, `c_level_only` → `c_level_score` (D-0820-12). Definition of the per-person aggregate (mean over that person's non-empty cells, `c_level_only` cells included when scored) is this brief's call — the matrix has no per-person total to copy |
