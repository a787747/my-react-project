#!/usr/bin/env python3
"""Apply the 20 H1 catalogue wording fields on LIVE via manage-criteria.

Brief CATALOGUE_FIX_H1_2026-08-25. Writes only through
...
write. Stops on 409/422 or any field outside the 20 changing.
...
BACKUP_DIR = REPO / "backups/2026-08-25-catalogue-fix"
CATALOGUE_DIR = REPO / "docs/catalogue"
VPS_TMP = "/root/epe_stand_tmp"

ADMIN_ID = 2
ADMIN_JTI = "caf10000-2026-0825-8000-000000000001"

WRITE_ORDER = (13, 3, 4, 8, 2, 10, 12, 14)
WRITABLE = [
...
def load_patches(path: Path) -> dict[int, dict[str, str]]:
    ...
    for key, fields in data.items():
        ...
        for field, value in fields.items():
            ...
            total += 1
    if total != 20:
        raise SystemExit(f"texts file must contain exactly 20 fields, got {total}")
    return patches
...
def main() -> None:
    ...
    parser.add_argument("--texts", type=Path,
                        default=REPO / "docs/briefs/catalogue_fix_h1_texts.json")
    parser.add_argument("--base-url", default="https://epe.sedamedical.com/webhook")
    parser.add_argument("--output", type=Path,
                        default=BACKUP_DIR / "catalogue_fix_h1_proof.json")
    ...
    before_path.write_text(snapshot_markdown(before_rows, before_ts, "before"), encoding="utf-8")
    REPORT["before"] = {
        "timestamp_utc": before_ts,
        "path": str(before_path),
        "equals_review_2026_08_24_19_22_58Z": True,  # measured this session before the run
        "rows": before_rows,
    }
    ...
    try:
        ...
        check("other 88 text fields identical", other_text_ok, 88)
        ...
        REPORT["forms_read"] = {"status": status, "criteria_3_13_14": read_check}
        ...
        # VPS dump removed at the end of a successful or failed run once the
        # local copy exists. Keep the VPS file only if the local copy is missing.
    ...
    # Do not persist the full row dumps twice in the proof — keep paths.
    slim = dict(REPORT)
    ...
    print("CATALOGUE FIX APPLIED AND PROVEN ON LIVE")
