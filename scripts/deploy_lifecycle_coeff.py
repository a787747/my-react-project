#!/usr/bin/env python3
"""Validate or PUT the workflows changed by the 2026-08-22 lifecycle brief.

Same contract as deploy_periods_hierarchy.py, extended to a set:
  ...
  - with --apply the tracked top-level exports are rewritten from live so
    n8n_workflows/API_*.json stays fresh (BUG-028 hygiene)
...
# generated file  ->  (live workflow id, tracked top-level export)
TARGETS: dict[str, tuple[str, str, str]] = {
    # builder key: h1 / def / auth
    "manage-periods.json":        ("h1",   "M9ljMDdO1mIl8m1h", "API_ Manage Periods.json"),
    "criteria.json":              ("h1",   "KKlGLEYMlXlbYUjb", "API_ Get Criteria With Levels.json"),
    "score-coefficients.json":    ("h1",   "zq3dufVhcnjkS7RV", "API_ Get Score Coefficients.json"),
    "save-score-coefficients.json": ("h1", "jAqkljoRb24jrcZx", "API_ Save Score Coefficients.json"),
    "submit-evaluation.json":     ("h1",   "tUxHoRn38rJVDxWv", "API_ Submit Evaluation.json"),
    "self-review-submit.json":    ("h1",   "CuHkTYvGDyhqEarg", "API_ Submit Self Review.json"),
    "update-evaluation.json":     ("h1",   "LWuZNTehzMDJkE8u", "API_ Update Evaluation WITH PERIOD.json"),
    "check-self-review.json":     ("h1",   "QRkUvs24DkcC3WBW", "API_ Check Self Review.json"),
    "check-evaluated.json":       ("h1",   "msl2T1flMo1Hn7uj", "API_ Check Evaluated V2.json"),
    "get-my-manager.json":        ("h1",   "3C1u68KOTSMwcqgy", "API_ Get My Manager.json"),
    "manage-criteria.json":       ("def",  "55BHbXWIS6igHHBT", "API_ Manage Criteria Admin V7.json"),
    "update-admin-data.json":     ("def",  "CkxIyrEJBrc6V4Cv", "API_ Update Admin Data.json"),
    "score-correction.json":      ("def",  "rSZcm0HDMUHLYk8W", "API_ Score Correction.json"),
    "protected-employees.json":   ("auth", "bKB4Sb46yWoq1tSV", "API_ Get Employees (Smart Role Based).json"),
}
...
def assert_not_a_generator_input(export_name: str) -> None:
    ...
    `build_route_guard_deferred.py` lifts legacy Code/SQL nodes straight out of
    the tracked top-level exports. Refreshing one of those FROM LIVE after the
    live graph has been replaced by the generated one deletes the very node the
    generator looks for, and the generator can never run again. Found the hard
    way on 2026-08-22 with `API_ Manage Criteria Admin V7.json` (`Prep SQL`),
    which is why that dependency is now inlined.
    """
    sources = [
