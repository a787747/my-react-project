#!/usr/bin/env bash
# Throwaway stand for ADMIN_USERS_SUMMARY_AND_CLEVEL_MODAL (2026-08-27).
# Fresh dump of live → epe_adminusers_<stamp> → isolated n8n on :25679.
...
DB="epe_adminusers_${STAMP}"
DUMP_NAME="epe_2026_adminusers_${STAMP}.dump"
BACKUP_DIR="$REPO/backups/2026-08-27-admin-users-summary"
IMAGE="n8nio/n8n@sha256:0a65e6e5995c19e0cf7e83d6b08ffa6c1898e8a53ff1658e6e7b22e68576c673"
CONTAINER="epe-adminusers-n8n"
PORT=25679
...
echo "== 3. Walkthrough fixture + campaign seed =="
sed 's/\^epe_walk_/\^epe_adminusers_/' "$REPO/scripts/seed_walkthrough_throwaway.sql" \
...
"${SSH[@]}" "docker exec -i postgres_n8n psql -U admin -d $DB -v ON_ERROR_STOP=1" \
  < "$REPO/scripts/seed_adminusers_evals.sql" >/dev/null
E="$("${SSH[@]}" "docker exec postgres_n8n psql -U admin -d $DB -tAc 'SELECT count(*) FROM performance_db.evaluations'")"
test "$E" != "0" || { echo "eval seed left evaluations empty"; exit 1; }
echo "  fixture 10, evaluations=$E"
...
test -f "$GEN/admin-users-data.json" || { echo "surface incomplete"; exit 1; }
...
{"database": "$DB", "jwt_secret": "$JWT_SECRET",
 ...
 "brief": "ADMIN_USERS_SUMMARY_AND_CLEVEL_MODAL"}
EOF
...
echo "  teardown  : scripts/teardown_adminusers_throwaway.sh"
