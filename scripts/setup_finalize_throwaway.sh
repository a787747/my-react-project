#!/usr/bin/env bash
# Build the isolated finalization proof stand on the VPS (brief 2026-08-24:
# corrections applicability, BUG-046 middle-manager matrix, new-criterion path):
#   - dated dump of live epe_2026
#   - throwaway DB epe_final_<stamp> restored from it (no migration needed:
#     this brief changes no schema)
#   - fixture seed applied to the throwaway ONLY
#   - isolated n8n `epe-final-n8n` on VPS loopback :25679 (same pinned image),
#     with the guard + every workflow this brief exercises imported
...
"${SSH[@]}" "docker exec postgres_n8n pg_dump -U admin -Fc epe_2026 > /tmp/epe_2026_final_${STAMP}.dump && ls -la /tmp/epe_2026_final_${STAMP}.dump"
"${SSH[@]}" "cat /tmp/epe_2026_final_${STAMP}.dump" > "$BACKUP_DIR/epe_2026_final_${STAMP}.dump"
echo "local copy: $BACKUP_DIR/epe_2026_final_${STAMP}.dump ($(wc -c < "$BACKUP_DIR/epe_2026_final_${STAMP}.dump") bytes)"

echo "== 2. Throwaway DB $DB restored from the dump =="
"${SSH[@]}" "docker exec postgres_n8n createdb -U admin $DB"
"${SSH[@]}" "docker exec -i postgres_n8n pg_restore -U admin -d $DB --no-owner < /tmp/epe_2026_final_${STAMP}.dump" || true
...
"${SSH[@]}" "docker exec -i postgres_n8n psql -U admin -d $DB -v ON_ERROR_STOP=1" < "$REPO/scripts/seed_finalize_throwaway.sql"
...
prep "$GEN_DEF/manager-subordinates-matrix.json"    manager-subordinates-matrix.json
...
scp -q "$CRED_FILE" "$HOST:/tmp/epe_final_cred.json"
# docker cp NESTS a directory when the target exists — clear the container path first.
tar -C "$IMPORT_DIR" -cf - . | "${SSH[@]}" "rm -rf /tmp/epe_fn_wf && mkdir -p /tmp/epe_fn_wf && tar -C /tmp/epe_fn_wf -xf -"
"${SSH[@]}" "docker exec $CONTAINER rm -rf /tmp/wf 2>/dev/null || true"
"${SSH[@]}" "docker cp /tmp/epe_final_cred.json $CONTAINER:/tmp/cred.json && docker cp /tmp/epe_fn_wf $CONTAINER:/tmp/wf && docker exec -u root $CONTAINER chown -R node:node /tmp/cred.json /tmp/wf"
...
"${SSH[@]}" "rm -f /tmp/epe_final_cred.json && rm -rf /tmp/epe_fn_wf"
...
for NAME in \
  ...
  "API: Manage Criteria Admin V7"; do
