#!/usr/bin/env bash
# Build the isolated two-gate-lifecycle proof stand on the VPS:
#   - dated dump of live epe_2026 (also the pre-migration-014 record)
#   - throwaway DB epe_lifecycle_<stamp> restored from it
#   - migration 014 + fixture seed applied to the throwaway ONLY
#   - isolated n8n `epe-lifecycle-n8n` on VPS loopback :25679 (same pinned image),
...
DB="epe_lifecycle_${STAMP}"
BACKUP_DIR="$REPO/backups/2026-08-22-lifecycle-coeff"
IMAGE="n8nio/n8n@sha256:0a65e6e5995c19e0cf7e83d6b08ffa6c1898e8a53ff1658e6e7b22e68576c673"
CONTAINER="epe-lifecycle-n8n"
CRED_ID="VNbfkY8IKbEzn88B"
GUARD_ID="L0Zr7nVa8O5YWXd3"

mkdir -p "$BACKUP_DIR"

echo "== 1. Dated dump of live epe_2026 (pre-migration-014 record) =="
"${SSH[@]}" "docker exec postgres_n8n pg_dump -U admin -Fc epe_2026 > /tmp/epe_2026_pre014_${STAMP}.dump && ls -la /tmp/epe_2026_pre014_${STAMP}.dump"
"${SSH[@]}" "cat /tmp/epe_2026_pre014_${STAMP}.dump" > "$BACKUP_DIR/epe_2026_pre014_${STAMP}.dump"
echo "local copy: $BACKUP_DIR/epe_2026_pre014_${STAMP}.dump ($(wc -c < "$BACKUP_DIR/epe_2026_pre014_${STAMP}.dump") bytes)"

echo "== 2. Throwaway DB $DB restored from the dump =="
"${SSH[@]}" "docker exec postgres_n8n createdb -U admin $DB"
"${SSH[@]}" "docker exec -i postgres_n8n pg_restore -U admin -d $DB --no-owner < /tmp/epe_2026_pre014_${STAMP}.dump" || true
...
echo "== 3. Migration 014 + seed on the throwaway =="
"${SSH[@]}" "docker exec -i postgres_n8n psql -U admin -d $DB -v ON_ERROR_STOP=1" < "$REPO/migrations/014_add_evaluation_start_gate.sql"
"${SSH[@]}" "docker exec -i postgres_n8n psql -U admin -d $DB -v ON_ERROR_STOP=1" < "$REPO/scripts/seed_lifecycle_throwaway.sql"
...
prep "$GEN_H1/criteria.json"                criteria.json
prep "$GEN_H1/score-coefficients.json"      score-coefficients.json
...
prep "$GEN_H1/admin-users-data.json"        admin-users-data.json
prep "$GEN_DEF/manage-criteria.json"        manage-criteria.json
prep "$GEN_DEF/update-admin-data.json"      update-admin-data.json
...
scp -q "$CRED_FILE" "$HOST:/tmp/epe_lifecycle_cred.json"
# docker cp NESTS a directory when the target exists — clear the container path first.
tar -C "$IMPORT_DIR" -cf - . | "${SSH[@]}" "rm -rf /tmp/epe_lc_wf && mkdir -p /tmp/epe_lc_wf && tar -C /tmp/epe_lc_wf -xf -"
"${SSH[@]}" "docker exec $CONTAINER rm -rf /tmp/wf 2>/dev/null || true"
"${SSH[@]}" "docker cp /tmp/epe_lifecycle_cred.json $CONTAINER:/tmp/cred.json && docker cp /tmp/epe_lc_wf $CONTAINER:/tmp/wf && docker exec -u root $CONTAINER chown -R node:node /tmp/cred.json /tmp/wf"
...
"${SSH[@]}" "rm -f /tmp/epe_lifecycle_cred.json && rm -rf /tmp/epe_lc_wf"
