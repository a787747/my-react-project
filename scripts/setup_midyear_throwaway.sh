#!/usr/bin/env bash
# Build the isolated MID_YEAR_HIRES_SCOPE stand on the VPS (brief 2026-08-25).
...
#       epe_mid_<stamp>_ctl   control   — nobody is excluded, then closed
#       epe_mid_<stamp>_trt   treatment — 1603 excluded by admin, then closed
...
#     byte-identical.
#   - migrations 015 and 016 applied to both (additive; the only schema change)
#   - scripts/seed_midyear_throwaway.sql applied to both (synthetic ids
#     1601-1611 with REAL scrypt logins: an excluded person must still get in)
#   - isolated n8n `epe-mid-n8n` on VPS loopback :25679, same pinned image
#     digest as live, pointed at the TREATMENT database, carrying the FULL
#     generated workflow surface including the new API: Manage Period Scope
set -euo pipefail
...
DB_CTL="epe_mid_${STAMP}_ctl"
DB_TRT="epe_mid_${STAMP}_trt"
BACKUP_DIR="$REPO/backups/2026-08-25-midyear-scope"
...
"${SSH[@]}" "mkdir -p $STAND_TMP && chmod 700 $STAND_TMP && docker exec postgres_n8n pg_dump -U admin -Fc --no-owner --no-acl epe_2026 > $STAND_TMP/epe_2026_mid_${STAMP}.dump && chmod 600 $STAND_TMP/epe_2026_mid_${STAMP}.dump && ls -la $STAND_TMP/epe_2026_mid_${STAMP}.dump"
"${SSH[@]}" "cat $STAND_TMP/epe_2026_mid_${STAMP}.dump" > "$BACKUP_DIR/epe_2026_mid_${STAMP}.dump"
echo "local copy: $BACKUP_DIR/epe_2026_mid_${STAMP}.dump ($(wc -c < "$BACKUP_DIR/epe_2026_mid_${STAMP}.dump") bytes)"
...
for DB in "$DB_CTL" "$DB_TRT"; do
  ...
  "${SSH[@]}" "docker exec -i postgres_n8n pg_restore -U admin -d $DB --no-owner < $STAND_TMP/epe_2026_mid_${STAMP}.dump" || true
...
echo "== 3. Migrations 015 + 016 on both (additive only) =="
for DB in "$DB_CTL" "$DB_TRT"; do
  for M in 015_add_employment_termination 016_add_period_scope_events; do
    "${SSH[@]}" "docker exec -i postgres_n8n psql -U admin -d $DB -v ON_ERROR_STOP=1" \
      < "$REPO/migrations/$M.sql" >/dev/null
    # Second application must be a no-op: the migration claims idempotence.
    "${SSH[@]}" "docker exec -i postgres_n8n psql -U admin -d $DB -v ON_ERROR_STOP=1" \
      < "$REPO/migrations/$M.sql" >/dev/null
    echo "  $DB: migration $M applied twice, no error"
  done
done
...
test -f "$GEN_H1/manage-period-scope.json" || { echo "manage-period-scope.json was not generated"; exit 1; }
...
{"database_control": "$DB_CTL", "database_treatment": "$DB_TRT", "jwt_secret": "$JWT_SECRET", "container": "$CONTAINER", "stamp": "$STAMP", "fixture_password": "Mid2026-Portal!"}
EOF
...
echo "  teardown     : scripts/teardown_midyear_throwaway.sh"
