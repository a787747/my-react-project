#!/usr/bin/env bash
# Build the isolated PRELAUNCH_BATCH_NIGHT stand on the VPS (2026-08-25/26).
...
#
#   epe_mid_night_<stamp>_ctl  ← container epe-night-n8n-old (:25680)
#                                the workflow surface AS COMMITTED AT HEAD
#   epe_mid_night_<stamp>_trt  ← container epe-night-n8n-new (:25679)
...
DB_CTL="epe_mid_night_${STAMP}_ctl"
DB_TRT="epe_mid_night_${STAMP}_trt"
BACKUP_DIR="$REPO/backups/2026-08-25-prelaunch-night"
IMAGE="n8nio/n8n@sha256:0a65e6e5995c19e0cf7e83d6b08ffa6c1898e8a53ff1658e6e7b22e68576c673"
CONTAINER_NEW="epe-night-n8n-new"
CONTAINER_OLD="epe-night-n8n-old"
PORT_NEW=25679
PORT_OLD=25680
...
"${SSH[@]}" "mkdir -p $STAND_TMP && chmod 700 $STAND_TMP && docker exec postgres_n8n pg_dump -U admin -Fc --no-owner --no-acl epe_2026 > $STAND_TMP/epe_2026_night_${STAMP}.dump && chmod 600 $STAND_TMP/epe_2026_night_${STAMP}.dump"
"${SSH[@]}" "cat $STAND_TMP/epe_2026_night_${STAMP}.dump" > "$BACKUP_DIR/epe_2026_night_${STAMP}.dump"
echo "local copy: $BACKUP_DIR/epe_2026_night_${STAMP}.dump ($(wc -c < "$BACKUP_DIR/epe_2026_night_${STAMP}.dump") bytes)"
...
for DB in "$DB_CTL" "$DB_TRT"; do
  ...
  "${SSH[@]}" "docker exec -i postgres_n8n pg_restore -U admin -d $DB --no-owner < $STAND_TMP/epe_2026_night_${STAMP}.dump" || true
...
echo "== 3. Fixture seed on both (midyear 1601-1611, then night 1612-1615) =="
for DB in "$DB_CTL" "$DB_TRT"; do
...
echo "  teardown     : scripts/teardown_night_throwaway.sh"
