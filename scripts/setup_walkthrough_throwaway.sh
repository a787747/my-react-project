#!/usr/bin/env bash
# Build the isolated browser-walkthrough stand on the VPS (brief 2026-08-24:
# browser-driven walkthrough of the campaign UI):
#   - dated dump of live epe_2026
#   - throwaway DB epe_walk_<stamp> restored from it (no schema change)
#   - fixture seed applied to the throwaway ONLY (real scrypt logins:
#     the browser drives the actual login form)
#   - isolated n8n `epe-walk-n8n` on VPS loopback :25679 (same pinned image),
#     with the guard + the FULL generated workflow surface imported: the
#     browser exercises every screen, so every frontend endpoint must answer
...
test "$CRIT_COUNT" = "9" || { echo "restore verification failed: active criteria=$CRIT_COUNT (criterion 9 must be in the dump)"; exit 1; }
echo "restore verified: users=$USER_COUNT, active criteria=$CRIT_COUNT"

echo "== 3. Fixture seed on the throwaway =="
"${SSH[@]}" "docker exec -i postgres_n8n psql -U admin -d $DB -v ON_ERROR_STOP=1" < "$REPO/scripts/seed_walkthrough_throwaway.sql"
...
# walkthrough does not cover registration and the stand has no SMTP credential.
...
while IFS= read -r NAME; do
  ...
  # loop ends after one name (cost this stand a re-activation pass).
