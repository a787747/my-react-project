#!/usr/bin/env bash
# Remove the CLEVEL_AVERAGING stand. The drop loop refuses any database
...
PREFIX="epe_mid_night_clv_"

for C in epe-clevel-n8n-new epe-clevel-n8n-old; do
...
for DB in $DBS; do
  case "$DB" in
    "${PREFIX}"*) ;;
    *) echo "refusing to drop '$DB': not a c-level stand"; exit 1;;
  esac
