#!/usr/bin/env python3
"""Verify the TERMINATED_EMPLOYEES deployment on LIVE — read-only (D-0825-7).

This session deploys the capability and terminates NOBODY. The point of this
script is therefore the negative claim: prove that adding the capability moved
no person's data, no money input and no period state.
...
ADMIN_ID = 2
PROBE_JTI = "caf10000-2026-0825-8000-000000000007"
...
NEW_COLUMNS = ["terminated_at", "termination_date"]
...
def main() -> None:
    ...
    parser.add_argument("--out", default=str(REPO / "backups/2026-08-25-termination/live_verify.json"))
    args = parser.parse_args()

    stamp = time.strftime("%Y%m%d%H%M%S", time.gmtime())
    throwaway = f"epe_termverify_{stamp}"
    ...
    try:
        ...
        anchor_users = users(throwaway, with_new_columns=False)
        ...
        check("89 people x every pre-existing column were compared",
        ...
        # The two new columns exist on live, are NULL on all 89, and were not
        # in the anchor at all.
        check("the new columns are absent from the anchor",
              [c for c in NEW_COLUMNS if c in columns], [])
        non_null = [uid for uid, row in live_users.items()
                    if row.get("terminated_at") is not None or row.get("termination_date") is not None]
        check("nobody on live is terminated", non_null, [])
    ...
    check("the employment event log is empty — nobody was terminated",
          int(sql("epe_2026", "SELECT count(*) FROM performance_db.employment_events")), 0)
    ...
    try:
        ...
        check("every row now carries the termination columns",
              sorted({("terminated_at" in r) and ("termination_date" in r) for r in rows}), [True])
        check("and every one of them is null",
              sorted({(r["terminated_at"], r["termination_date"]) for r in rows}), [(None, None)])
        ...
        REPORT["employment_events"] = {"status": status, "body": body}
        check("the event log route answers 200", status, 200)
        check("and is empty", (body or {}).get("events"), [])
        ...
        check("still 87 in scope of 89", [h1["in_scope_count"], h1["participant_count"]], [87, 89])
